...
def select_representative(output_root: Path, candidates: list[int]) -> int:
    rates: dict[tuple[str, int], list[float]] = {}
    for path in sorted((output_root / "raw/repair_rate").glob("*/summary.csv")):
...
def main() -> int:
    ...
    for faults in manifest["fault_counts_per_group"]:
        for seed in profile["seeds"]:
            point_id = f"f{faults}_s{seed}"
            point_dir = output_root / "raw/repair_rate" / point_id
            command = [
                str(repair), str(fixed["rs"]), str(fixed["cs"]),
            ...
            run_logged(command, point_dir, {
                ...
                "scope": "group", "fault_count_per_group": faults,
                "seed": seed, "runs": profile["runs_per_seed"],
                "paired_policies": [policy["id"] for policy in manifest["policies"]],
            })

    representative = select_representative(
        output_root, manifest["imbalance"]["representative_candidates"])
    resolved["selected_representative_fault_count"] = representative
    write_json(output_root / "run_config.json", resolved)
    ...
    for distribution in manifest["imbalance"]["profiles"]:
        ...
        for seed in profile["seeds"]:
            ...
            for policy in manifest["policies"]:
                point_id = f"{corpus_id}_{policy['id']}"
                point_dir = output_root / "raw/imbalance" / point_id
    ...
    for seed in profile["seeds"]:
        point_id = f"f{representative}_s{seed}_directional_m2"
        point_dir = output_root / "raw/hardware_model" / point_id
        command = [
            str(sram), str(fixed["rs"]), str(fixed["cs"]),
