set design recam_shared_config_analyzer
...
source [file join $repo_root scripts synthesis dc recam_phase3b_sources.tcl]
analyze -format sverilog $recam_phase3b_sources
elaborate $design
...
# The structural fallback preserves the same technology, wire-load model, and
# virtual analyzer boundary while avoiding an environment-limited optimization
# run. Its reports must never be labeled timing-driven closure.
set timing_driven 1
if {[info exists ::env(TIMING_DRIVEN)] && $::env(TIMING_DRIVEN) eq "0"} { set timing_driven 0 }
if {$timing_driven} {
