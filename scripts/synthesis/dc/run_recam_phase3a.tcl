...
source [file join $repo_root scripts synthesis dc recam_phase3a_sources.tcl]
analyze -format sverilog $recam_phase3a_sources
elaborate $design
...
# Structural mapping intentionally preserves the approved fully-combinational
# architecture.  TIMING_DRIVEN=1 applies the same boundary constraints before
# compile_ultra; it never changes analyzer RTL or adds registers.
set timing_driven 0
if {[info exists ::env(TIMING_DRIVEN)] && $::env(TIMING_DRIVEN) eq "1"} { set timing_driven 1 }
if {$timing_driven} {
    source [file join $repo_root scripts synthesis dc recam_phase3a_constraints.tcl]
    if {[info exists ::env(TIMING_DRIVEN_FAST)] && $::env(TIMING_DRIVEN_FAST) eq "1"} {
